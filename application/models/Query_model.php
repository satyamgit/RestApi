...
//=============================================Function for Joins for multiple data==============================================================//
...
//**---JOIN DATA
 ...
 /*public function get_sql_select_data_following() {
 $where = array('tbl_userFallow.follow_des'=>'1','user.user_id' => $this->session->userdata('user_id'));
   $this->db->select('tbl_categorydesign.* , tbl_userFallow.*', 'user.*');
   $this->db->from('tbl_userFallow');
    $this->db->join('user','user.user_id = tbl_categorydesign.Uid');
    $this->db->join('tbl_categorydesign','tbl_categorydesign.designId = tbl_userFallow.designId');
    $this->db->where($where);
     $this->db->order_by('tbl_userFallow.designId', 'desc');
     $query = $this->db->get();
